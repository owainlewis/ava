...
import Language.Ava.Base.Error
       (ProgramError(..), failGeneric, failState)
...
liftModify
  :: Monad m
  => ([a] -> Either e [a]) -> Stack a -> ExceptT e m (Stack a)
...
define
  :: Monad m
  => String -> [a] -> Stack a -> ExceptT e m (Stack a)
...
letOp
  :: Monad m
  => AST.Op -> a -> Stack a -> ExceptT e m (Stack a)
...
cons s = liftModify f s
  where
    f ((Prim (Quotation xs)):x:ys) =
      return $ ((liftPrim (Quotation (x : xs))) : ys)
    f (Prim (List xs):x:ys) = return $ ((liftPrim (List (x : xs))) : ys)
...
uncons s = liftModify f s
  where
    f (Prim (AST.Quotation (x:xs)):ys) =
      return $ x : (liftPrim $ Quotation xs) : ys
    f (Prim (AST.List (x:xs)):ys) = return $ x : (liftPrim $ List xs) : ys
...
_stack :: AvaFunction
_stack s =
  liftExcept (Stack.modifyM (\xs -> return $ liftPrim (Quotation xs) : xs) s)
...
numericBinOp s op opName = liftModify f s
  where
    f (Prim (Integer x):(Prim (Integer y)):xs) =
